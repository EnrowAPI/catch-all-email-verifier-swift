...
public struct CatchAllVerifierError: Error, LocalizedError {
...
// MARK: - CatchAllVerifier

public struct CatchAllVerifier: Sendable {
    private static let baseURL = "https://api.enrow.io"

    // MARK: - Single

    /// Start a catch-all email verification. Returns a verification ID you can poll with `getResult`.
    ...
    private static func request<T: Decodable>(
        ...
        guard let url = URL(string: "\(baseURL)\(path)") else {
            throw CatchAllVerifierError(statusCode: 0, message: "Invalid URL: \(path)")
        }
        ...
        guard let http = response as? HTTPURLResponse else {
            throw CatchAllVerifierError(statusCode: 0, message: "Invalid response")
        }
        ...
        if !(200...299).contains(http.statusCode) {
            ...
            throw CatchAllVerifierError(statusCode: http.statusCode, message: errorMessage)
        }
